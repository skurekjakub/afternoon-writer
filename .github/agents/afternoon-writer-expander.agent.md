...
- **NO Scene break markers** — do not add new scene break markers (`~`, `---`, or whatever the style target uses) just because the outline moved to a new beat. Preserve real cuts. If the input already contains a break marker inside continuous action, same-room planning, same walk, or uninterrupted dialogue, close the gap instead of multiplying it.
